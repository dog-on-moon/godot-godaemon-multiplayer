...
steam_lib_path = "godaemon-cpp/steam-sdk/redistributable_bin"
...
env.Append(CPPPATH=['godaemon-cpp/steam-sdk/public'])
env.Append(LIBS=[
    steamworks_library.replace(".dll", "")
])

env.Append(CPPPATH=['godaemon-cpp/'])
sources = [
    Glob('godaemon-cpp/*.cpp'),
    ]
