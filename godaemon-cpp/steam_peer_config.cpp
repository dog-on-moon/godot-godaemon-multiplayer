...
void SteamPeerConfig::apply_options(const HSteamNetConnection hConn) {
	int options_size = options.size();
	if (options_size > 0) {
		for (int i = 0; i < options_size; i++) {
			SteamNetworkingConfigValue_t this_option;
			...
			if (type == Variant::INT) {
				if (sent_option != k_ESteamNetworkingConfig_ConnectionUserData) {
					SteamNetworkingUtils()->SetConnectionConfigValueInt32(hConn, this_value, options[sent_option]);
				}
			} else if (type == Variant::FLOAT) {
				SteamNetworkingUtils()->SetConnectionConfigValueFloat(hConn, this_value, options[sent_option]);
			} else if (type == Variant::STRING) {
				char *this_string = { 0 };
				String passed_string = options[sent_option];
				strcpy(this_string, passed_string.utf8().get_data());
				SteamNetworkingUtils()->SetConnectionConfigValueString(hConn, this_value, this_string);
			}
			option_array[i] = this_option;
		}
	}
