...
static func mp(node: Node) -> MultiplayerRoot:
	return api(node).mp
...
static func api(node: Node) -> GodaemonMultiplayerAPI:
	return node.multiplayer

## The API profiler allows configuring Godot's network profiler.
static func profiler(node: Node) -> Profiler:
...
static func repository(node: Node) -> Repository:
...
static func replication_service(node: Node) -> ReplicationService:
...
static func sync_service(node: Node) -> SyncService:
